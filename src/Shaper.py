...
class Shaper(nn.Module):
    ...
    def __init__(self, observables, batch_size = None) -> None:
        super().__init__()
    ...
    def calculate(self, events, epochs = 500, early_stopping = 25, lr = 0.05, N = 250, epsilon = 0.01, scaling = 0.5, verbose = False, plot_directory = None, gif_directory = None):
        ...
        for obs in self.observables:
            ...
            if gif_directory is not None:
                fs = self.plot(events, obs, losses, gif_directory, gif_extention="init")
                for f in fs:
            ...
            if len(list(self.observables[obs].parameters())) == 0 or epochs == 0:
                ...
                for epoch in range(epochs):
                    ...
                    # If gif plotter is on, save gif plots
                    if gif_directory is not None:
                        fs = self.plot(events, obs, losses, gif_directory, gif_extention="%d" % epoch)
                        for i in range(batch_size):
                            gif_filenames[i].append(fs[i])

                    if verbose:
                        print("Observable:", obs,"Epoch %d" % epoch, "Mean Loss =" , np.mean(losses[obs]), "Elapsed time = %.3fs" % (time() - t1), "Percentage done = %.3f " % (100 * (batch_size - np.sum(mask))/batch_size))
            ...
            # Plotting
            if plot_directory:    
                self.plot(events, obs, losses, plot_directory)
            if gif_directory:
                self.make_gifs(gif_filenames, obs, gif_directory)
    ...
    def plot(self, events, obs, losses, directory, gif_extention = ""):
        ...
        # Make directory is it doesn't exist
            ...
            for i in range(len(events)):
                ...
                filename = os.path.join(dir, "event_%d.png" % i)
