...
def plot(y_i, z_i, y_j, z_j, x_ij, loss,  R, z0, filename, name, shapes, color = "red"):
    ...
    for shape in shapes:
        ...
        if shape.name == "Uniform":
            has_background = True
            ax.scatter(ys, phis, marker='o', s=2 * pts * 500/np.sum(pts) , color="royalblue", lw=0, zorder=10, label="Background")
        elif flag and shape.name != "Disk":
            ax.scatter(ys, phis, marker='o', s=2 * pts * 500/np.sum(pts) , color="purple", lw=0, zorder=10, label="Structure Points")
            # flag = False
...
def plot_event(y_i, z_i, R, filename = None, graph = None, A = None, color = "red"):
    ...
    pts, ys, phis = z_i, y_i[:,0], y_i[:,1]
    ax.scatter(ys, phis, marker='o', s=2 * pts * 500/np.sum(pts), color= "red", lw=0, zorder=10, label="Event")
